...
s/  static func newWith(with_label label: UnsafePointer<gchar>) -> ToggleButton/  override static func newWith(with_label label: UnsafePointer<gchar>) -> ToggleButton/
...
s/\(public func dragSetIconStock\)/@available(*, deprecated) \1/
...
s/@available(., deprecated) @available(., deprecated)/@available(*, deprecated)/g
s/let rv = gtk_cell_area_get_cell_at_position(cast.*ptr))$/let ptrPtr: UnsafeMutablePointer<GtkCellArea> = cast(ptr)\n        let contextPtr: UnsafeMutablePointer<GtkCellAreaContext>  = cast(context.ptr)\n        let widgetPtr: UnsafeMutablePointer<GtkWidget> = cast(widget.ptr)\n        let cellAreaPtr: UnsafePointer<GdkRectangle> = cast(cell_area.ptr)\n        let xPtr: gint = gint(x)\n        let yPtr: gint = gint(y)\n        let allocAreaPtr: UnsafeMutablePointer<GdkRectangle> = cast(alloc_area.ptr)\n        let rv = gtk_cell_area_get_cell_at_position(ptrPtr, contextPtr, widgetPtr, cellAreaPtr, xPtr, yPtr, allocAreaPtr)/
s/let rv = gtk_cell_renderer_start_editing(cast.*flags)$/let ptrPtr: UnsafeMutablePointer<GtkCellRenderer> = cast(ptr)\n        let eventPtr: UnsafeMutablePointer<GdkEvent> = cast(event)\n        let widgetPtr: UnsafeMutablePointer<GtkWidget> = cast(widget.ptr)\n        let backgroundAreaPtr: UnsafePointer<GdkRectangle> = cast(background_area.ptr)\n        let cellAreaPtr: UnsafePointer<GdkRectangle> = cast(cell_area.ptr)\n        let rv = gtk_cell_renderer_start_editing(ptrPtr, eventPtr, widgetPtr, path, backgroundAreaPtr, cellAreaPtr, flags)/
s/let rv = gtk_text_buffer_serialize(cast.*length))$/let ptrPtr: UnsafeMutablePointer<GtkTextBuffer> = cast(ptr)\n        let contentBufferPtr: UnsafeMutablePointer<GtkTextBuffer> = cast(content_buffer.ptr)\n        let formatPtr: GdkAtom = cast(format.ptr)\n        let startPtr: UnsafePointer<GtkTextIter> = cast(start.ptr)\n        let endPtr: UnsafePointer<GtkTextIter> = cast(end.ptr)\n        let lengthPtr: UnsafeMutablePointer<gsize> = cast(length)\n        let rv = gtk_text_buffer_serialize(ptrPtr, contentBufferPtr, formatPtr, startPtr, endPtr, lengthPtr)/
s/let rv = gtk_icon_info_load_symbolic(cast.*error)$/let ptrPtr: UnsafeMutablePointer<GtkIconInfo> = cast(ptr)\n        let fgPtr: UnsafePointer<GdkRGBA> = cast(fg.ptr)\n        let successColorPtr: UnsafePointer<GdkRGBA> = cast(success_color.ptr)\n        let warningColorPtr: UnsafePointer<GdkRGBA> = cast(warning_color.ptr)\n        let errorColorPtr: UnsafePointer<GdkRGBA> = cast(error_color.ptr)\n        let wasSymbolicPtr: UnsafeMutablePointer<gboolean> = cast(was_symbolic)\n        let rv = gtk_icon_info_load_symbolic(ptrPtr, fgPtr, successColorPtr, warningColorPtr, errorColorPtr, wasSymbolicPtr, \&error)/
